...
app.post('/api/chat', async (req, res) => {
    try {
        ...
        // 🚨 This will print the EXACT error stack to your VS Code terminal/Node log
        console.error("❌ BACKEND CRASH ERROR LOG:", error.stack || error);

        // Detect quota / rate-limit errors from the AI provider and return a clear status
        const isQuotaError = (
...
import { getDbConnection } from './database.js'; // Adjust path if needed
...
app.post('/api/shopping-list/batch', async (req, res) => {
    try {
        const { items } = req.body; // Expects an array: ["Milk", "Eggs", "Cheese"]
        ...
        // Loop through and insert each missing item cleanly using async statements
